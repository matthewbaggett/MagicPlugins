{$visitors = VisitorSearcher::Factory()->sort("id","asc")->execute()}
                        				...
                        				{foreach from=$page->data key=i item=row}
                        					<tr>
	                        					{foreach from=$row key=column_id item=column_value}
	                        						...
	                        						{if $column_id == 'visitor'}
	                        							<a href="/Manage/EditData/Visitor?id={$visitors[$column_value]->get_id()}">{$visitors[$column_value]->get_ip()}</a>
	                        						{elseif $column_id == 'accesstime'}
	                        							{date("Y/m/d H:i:s",$column_value)}
	                        						{else}
	                        							{$column_value}
	                        						{/if}
	                        						</td>
