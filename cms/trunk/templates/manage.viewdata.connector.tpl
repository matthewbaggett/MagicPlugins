{$connectortypes = ConnectorTypeSearcher::Factory()->sort("id","asc")->execute()}
                        				...
                        				{foreach from=$page->data key=i item=row}
                        					<tr>
	                        					{foreach from=$row key=column_id item=column_value}
	                        						...
	                        						{if $column_id == 'type'}
	                        							<a href="/Manage/EditData/ConnectorType?id={$connectortypes[$column_value]->get_id()}">{$connectortypes[$column_value]->get_name()}</a>
	                        						{else}
	                        							{$column_value}
	                        						{/if}
	                        						</td>
