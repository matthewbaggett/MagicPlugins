{$connectors = ConnectorSearcher::Factory()->sort("id","asc")->execute()}
                        				...
                        				{foreach from=$page->data key=i item=row}
                        					<tr>
	                        					{foreach from=$row key=column_id item=column_value}
	                        						...
	                        						{elseif $column_id == 'connector'}
	                        							<a href="/Manage/EditData/Connector?id={$connectors[$column_value]->get_id()}">{$connectors[$column_value]->get_name()}</a>
	                        						{else}
	                        							{$column_value}
	                        						{/if}
	                        						</td>
